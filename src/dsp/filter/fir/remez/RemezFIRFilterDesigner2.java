...
public class RemezFIRFilterDesigner2
{
    ...
    public double getFrequencyResponse(double cosineOfFrequency)
    {
        ...
        {
            BigDecimal cosineDelta = BigDecimal.valueOf(cosineOfFrequency -
                mGrid.getCosineFrequencyGrid()[mExtremalIndices.get(k)]);

            //If this frequency is close to one of the polynomial points, use c[k] for that point
            if(cosineDelta.abs().compareTo(BigDecimal.valueOf(1.0e-7)) < 0)
            {
                numerator = BigDecimal.valueOf(mIdealFrequencyResponse[k]);
                denominator = BigDecimal.ONE;
                break;
            }
            else
            {
                BigDecimal dkOverCosineDelta = mD[k].divide(cosineDelta, 25, RoundingMode.HALF_UP);
