...
public class ChannelizerViewer extends JFrame
{
    ...
    public ChannelizerViewer(int channelsPerRow)
    {
        ...
        mChannelCount = (int)(mTestTuner.getTunerController().getFrequencyController().getSampleRate() / CHANNEL_BANDWIDTH);
        mChannelsPerRow = channelsPerRow;

        mToneFrequency = mTestTuner.getTunerController().getFrequencyController().getFrequency() + 19200;
    ...
    private PrimarySpectrumPanel getSpectrumPanel()
    {
        if(mPrimarySpectrumPanel == null)
        {
            mPrimarySpectrumPanel = new PrimarySpectrumPanel(mSettingsManager,
                mTestTuner.getTunerController().getFrequencyController().getSampleRate());
    ...
    private JPanel getControlPanel()
    {
        if(mControlPanel == null)
        {
            ...
            int maximumFrequencyOffset = (int)(mTestTuner.getTunerController().getFrequencyController().getSampleRate() / 2);
            long minimumFrequency = mTestTuner.getTunerController().getFrequencyController().getFrequency() - maximumFrequencyOffset;
            long maximumFrequency = mTestTuner.getTunerController().getFrequencyController().getFrequency() + maximumFrequencyOffset;
            ...
            mControlPanel.add(new JLabel("Sample Rate: " +
                mTestTuner.getTunerController().getFrequencyController().getSampleRate() + " Hz"), "push,align right");

            mControlPanel.add(new JLabel("Max: " +
                (mTestTuner.getTunerController().getFrequencyController().getSampleRate() / 2) + " Hz"), "push,align right");
    ...
    public class ChannelArrayPanel extends JPanel
    {
        ...
        private void init()
        {
            ...
            double spectralBandwidth = mTestTuner.getTunerController().getFrequencyController().getSampleRate();
            ...
            for(int x = 0; x < mChannelCount; x++)
            {
                ...
                long frequency = (long)(mTestTuner.getTunerController().getFrequencyController().getFrequency() +
