...
import dsp.filter.channelizer.ComplexPolyphaseChannelizerM2;
...
import source.tuner.channel.polyphase.output.IPolyphaseChannelOutputProcessor;
import source.tuner.channel.polyphase.output.OneChannelOutputProcessor;
import source.tuner.channel.polyphase.output.TwoChannelOutputProcessor;
import source.tuner.channel.FrequencyController;

import java.util.List;
import java.util.concurrent.CopyOnWriteArrayList;

public class PolyphaseChannelManager extends ChannelManager
{
     ...
     *
     * @param frequencyController that can be controlled by this channel manager
     * @param complexBufferProvider that supports register/deregister of this channel manager
     */
    public PolyphaseChannelManager(FrequencyController frequencyController, IComplexBufferProvider complexBufferProvider)
    {
        super(frequencyController);
        ...
        if(getFrequencyController().getSampleRate() % (CHANNEL_SAMPLE_RATE) != 0)
        {
            throw new IllegalArgumentException("Tuner sample rate [" + getFrequencyController().getSampleRate() +
                "] must be an even multiple of " + CHANNEL_SAMPLE_RATE + " Hz");
        }

        int channelCount = (int)(getFrequencyController().getSampleRate() / CHANNEL_BANDWIDTH);

        mChannelCalculator = new ChannelCalculator(getFrequencyController().getSampleRate(), channelCount,
            getFrequencyController().getFrequency(), CHANNEL_OVERSAMPLING);

        mBufferProcessor = new ScheduledBufferProcessor(500, 100, 50, 50);

        updateChannelizer(getFrequencyController().getSampleRate(), null);
    }

    @Override
    protected long calculateCenterFrequency()
    {
        return 0;
    }
     ...
     */
    @Override
    protected TunerChannelSource getTunerChannelSource(TunerChannel tunerChannel)
    {
        ...
        try
        {
            ...
            if(outputProcessor != null)
            {
                ...
                channelSource.setFrequency(mChannelCalculator.getCenterFrequencyForIndexes(polyphaseIndexes));
    ...
    private void stopChannelSource(PolyphaseChannelSource channelSource)
    {
        synchronized(mBufferProcessor)
        {
            super.releaseSource(channelSource);
