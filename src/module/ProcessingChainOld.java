...
import source.tuner.frequency.AutomaticFrequencyControl_R;
 ...
 */
public class ProcessingChainOld implements Listener<Message>
{
	private final static Logger mLog = LoggerFactory.getLogger( ProcessingChainOld.class );
	...
	protected AutomaticFrequencyControl_R mAFC;
	protected ChannelState mChannelState;
	protected ResourceManager mResourceManager;
	private ScheduledFuture<?> mSampleProcessorTask;
	private AliasList mAliasList;

	private ComplexSampleReceiver mComplexReceiver = new ComplexSampleReceiver();
	private LinkedTransferQueue<List<Complex>> mComplexQueue = 
								new LinkedTransferQueue<List<Complex>>();
	...
	public ProcessingChainOld( Channel channel, ResourceManager resourceManager )
	{
		mChannel = channel;
...
//	}
	...
	public Listener<List<Complex>> getComplexReceiver()
	{
	...
	public void dispose()
	{
		if( mSampleProcessorTask != null )
		{
			mResourceManager.getThreadPoolManager().cancel( mSampleProcessorTask );
		}
...
//				getChannelState().getCallEventModel().flush();
	...
	private void releaseSource()
	{
		if( mSource != null )
		{
			...
			{
//				((ComplexSource)mSource).removeAttributeListener( this.getComplexReceiver() );
			}
...
//	            					.getSource( ProcessingChainOld.this );
//            }
...
//				mChannelState = DecoderFactory.getChannelStateNew( this, mAliasList );
...
//							getDecoder( this, mSource.getSampleType(), mAliasList );
					
					if( mDecoder != null )
					{
						/* Register to receive decoded messages and auxiliary messages */
						mDecoder.addMessageListener( ProcessingChainOld.this );
...
//									mDecoder.getFrequencyCorrectionController();
//							
//							if( fcc != null )
//							{
//								tcs.addListener( fcc );
...
//			mDecoder.addRealSampleListener( listener );
    ...
    public void removeRealListener( RealSampleListener listener )
    {
		if( mDecoder != null )
		{
//			mDecoder.removeRealSampleListener( listener );
		}
    }
     ...
     */
    public void addComplexListener( Listener<Complex> listener )
    {
		if( mDecoder != null )
		{
//			mDecoder.addComplexListener( listener );
     ...
     */
    public void removeComplexListener( Listener<Complex> listener )
    {
		if( mDecoder != null )
		{
...
//							mDecoder.getRealReceiver().receive( sample );
	...
	private class ComplexProcessor implements Runnable
	{
        ...
        public void run()
        {
			try
			{
				List<List<Complex>> sampleSets = 
						new ArrayList<List<Complex>>();

				mComplexQueue.drainTo( sampleSets, 16 );
			
				for( List<Complex> samples: sampleSets )
				{
...
//							mDecoder.getComplexReceiver().receive( sample );
	 ...
	 */
	public class ComplexSampleReceiver implements Listener<List<Complex>>
	{
		@Override
