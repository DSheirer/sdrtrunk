...
public class PhaseAwareLinearInterpolator
{
    ...
    public static float filter(float[] buffer, int bufferPointer, float mu)
    {
//        return calculate(buffer[bufferPointer], buffer[bufferPointer + 1], mu);
//        return calculate(buffer[bufferPointer + 1], buffer[bufferPointer + 2], mu);
//        return calculate(buffer[bufferPointer + 2], buffer[bufferPointer + 3], mu);
//        return calculate(buffer[bufferPointer + 3], buffer[bufferPointer + 4], mu);
//        return calculate(buffer[bufferPointer + 4], buffer[bufferPointer + 5], mu);
        return calculate(buffer[bufferPointer + 5], buffer[bufferPointer + 6], mu);
    }
