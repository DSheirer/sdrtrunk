...
public class Slicer implements Listener<Boolean>
{
	...
	public void receive( Boolean sample )
	{
		...
		if( mSampleCounter >= mSymbolLength )
		{
			...
			{
				sendTapEvent( mBitSet, SymbolEvent.Shift.LEFT, decision );
			 ...
			 * decision and the right bit is opposite */
					 ...
					 ( mBitSet.get( mSymbolLength - 1 ) ^ decision ) )
			{
				sendTapEvent( mBitSet, SymbolEvent.Shift.RIGHT, decision );
			...
			{
				sendTapEvent( mBitSet, SymbolEvent.Shift.NONE, decision );
	 ...
	 */
	private void sendTapEvent(BitSet bitset, SymbolEvent.Shift shift, boolean decision )
	{
