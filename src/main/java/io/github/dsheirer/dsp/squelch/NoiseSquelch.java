 ...
 *
 * Register an audio listener to receive squelch controlled audio and a squelch state listener to receive squelch
 * toggle indications.
 */
public class NoiseSquelch implements INoiseSquelchController
{
    ...
    public static final float DEFAULT_NOISE_CLOSE_THRESHOLD = 0.2f;
    public static final float MINIMUM_NOISE_THRESHOLD = 0.0f;
    public static final float MAXIMUM_NOISE_THRESHOLD = 1.0f;
    ...
    private int mSquelchOpenIndex = -1;
    ...
    private long mDebugSampleCounter = 0;
    ...
    public void setNoiseThreshold(float open, float close)
    {
        ...
        {
            throw new IllegalArgumentException("Noise threshold open [" + open + "] must be less than or equal to close [" +
                    close + "] and both in range " + MINIMUM_NOISE_THRESHOLD + "-" + MAXIMUM_NOISE_THRESHOLD);
        }
    ...
    {
        System.out.println("Noise squelch - registering listener: " + listener);
     ...
     * in concert with processed audio buffers.  An unsquelch event is followed by one or more audio buffers and then a
     * squelch event.
     * @param listener to register or de-register by passing in a null.
     */
    ...
    /**
     * Registers the listener to receive processed audio buffers in between unsquelch/squelch events.
     * @param listener to register or de-register by passing in a null.
     */
     ...
     * indices are pointers to the filtered audio buffer and we dispatch from the unfilterd audio buffer which is offset
    ...
    private void broadcast(int start, int end)
    {
        if(mDebug)
        {
            System.out.println("\t\t**Broadcasting Audio from buffer [" + (mDebugSampleCounter + start) +
                    " / " + (mDebugSampleCounter + end - 1) + "] using offsets [" + start + "/" + end + "]");
        }
    ...
    private void broadcast(SquelchState squelchState)
    {
        if(mDebug)
        {
            System.out.println("\t\t**Broadcasting Squelch State: " + squelchState);
        }
    ...
    public void process(float[] samples)
    {
        ...
        int squelchCloseIndex = -1;
        //Adjust the squelch open index in sync with the left-shuffle of the audio samples.

        if(mSquelchOpenIndex >= 0 && mDebug)
        {
            System.out.println("Squelch open index shifting from [" + mSquelchOpenIndex + "] to [" + (mSquelchOpenIndex - samples.length) + "] for sample length [" + samples.length + "]");
        }

        mSquelchOpenIndex = Math.max(mSquelchOpenIndex - samples.length, mSquelch ? -1 : 0);
        ...
        for(int x = mFilteredBuffer.length - filtered.length; x < mFilteredBuffer.length; x++)
        {
            ...
            if(mMeanAccumulatorPointer == mVarianceWindowSize)
            {
                ...
                float varianceAccumulator1 = 0.0f, deviation = 0.0f, varianceAccumulator2 = 0.0f;
                ...
                if(mSquelch && mHysteresisCount >= mHysteresisOpenThreshold)
                {
                    ...
                    if(mDebug)
                    {
                        System.out.println("***************** UNSQUELCH *********************** AT: " + (mDebugSampleCounter + mSquelchOpenIndex));
                    }
                ...
                else if(!mSquelch && mHysteresisCount <= 0)
                {
                    ...
                    if(mDebug)
                    {
                        System.out.println("***************** SQUELCH *********************** AT: " + (mDebugSampleCounter + squelchCloseIndex) + " Dispatching [" + mSquelchOpenIndex + "/" + squelchCloseIndex + "]");
                    }
                    ...
                    else if(mDebug && !mSquelchOverride)
                    {
                        System.out.println("*** WARNING -- open index [" + mSquelchOpenIndex + "] is greater than close index [" + squelchCloseIndex + "]");
                    }
                    mSquelchOpenIndex = -1;
                }
                ...
                if(mDebug)
                {
                    System.out.println((mDebugSampleCounter + x) + "\tVariance: " + DECIMAL_FORMAT.format(noiseVariance) + "\tCount: " + mHysteresisCount + (below ? "\t\t<<<<<<<<<<<<<<<< BELOW" : ""));
                }
            }
        }
        ...
        {
            //Dispatch audio after processing the audio buffer if we end in an unsquelch state.
            broadcast(mSquelchOpenIndex, samples.length);
            mSquelchOpenIndex = -1;
        }

        mDebugSampleCounter += filtered.length;
    }
     ...
     * allowing noise to leak into the squelch period at the end of the transmission.
     *
     * Uses a reduced 20-sample variance calculation window extended to the hysteresis time period to detect the
     * transition point.  Identifies the transition point within +/- 20 audio samples.
     *
    ...
    private int findTransition(int current)
    {
        ...
        if(mDebug)
        {
            String transition = mSquelch ? "Un-Squelch --> Squelch" : "Squelch --> Un-Squelch";
            System.out.println("Finding Transition - current [" + current + "] start [" +  start + "] end [" + end + "] buffer size [" + mVarianceWindowSize + "]");
            System.out.println("\tTransition [" + transition + "] Window [" + window + "] thresholds - variance [" + varianceThreshold + "] hysteresis [" + hysteresisThreshold + "]");
        }
        ...
        for(int x = start; x < end; x += windowSize)
        {
            ...
            //Detect transition from from unsquelch to squelch
            ...
            if(mDebug)
            {
                System.out.println("\t" + x + " Variance: " + DECIMAL_FORMAT.format(variance) + " Hysteresis: " + hysteresisCount + (exceedsThreshold ? (mSquelch ? " <<< CLOSE SQUELCH" : " <<< OPEN SQUELCH") + " TRANSITION DETECTED AT: " + transitionPointer : "") + " THRESHOLD VARIANCE [" + varianceThreshold + "] HYSTERESIS [" + hysteresisThreshold + "]");
            }
        }
...
//        Path file = directory.resolve("20250703_043958_162550000_NOAA_25_khz_yes_center_signal.wav");
        ...
        config.setSquelchThreshold(-200);
        NBFMDecoder2 decoder = new NBFMDecoder2(config);
