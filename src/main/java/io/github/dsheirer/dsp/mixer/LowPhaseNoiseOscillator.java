...
public class LowPhaseNoiseOscillator extends AbstractOscillator
{
    ...
    /**
     * Sets the frequency of the tone generated by this oscillator.
     */
    @Override
    public void setFrequency(double frequency)
    {
        //This oscillator does not recover when the current frequency is equal to exactly half the sample rate, so we
        //have to set a flag to reset the I/Q structures when changing the frequency from exactly half the sample rate.
        mReset = getFrequency() < 0.01 || Math.abs(getFrequency()) - (getSampleRate() / 2) < 1.0;
    ...
    protected void update()
    {
        ...
        if(mReset)
        {
            mInphase = 1.0;
            mQuadrature = 0.0;
            mPreviousInphase = 1.0;
            mPreviousQuadrature = 0.0;
            mReset = false;
        }
    }
