...
public abstract class FullyQualifiedRadioIdentifier extends RadioIdentifier
{
     ...
     * @param persona radio identifier, aka alias.  This can be the same as the radio ID when the fully qualified radio
     ...
     */
    public FullyQualifiedRadioIdentifier(int persona, int wacn, int system, int id, Role role)
    {
        super(persona > 0 ? persona : id, role);
        mWacn = wacn;
        mSystem = system;
        mRadio = id;
    }
