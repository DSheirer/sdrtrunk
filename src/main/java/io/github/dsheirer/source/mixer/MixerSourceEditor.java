...
public class MixerSourceEditor extends Editor<Channel>
{
	...
	private void init()
	{
    	...
    	MixerManager.InputMixerConfiguration[] inputMixers = mSourceManager.getMixerManager().getInputMixers();
		mComboMixers.setModel( new DefaultComboBoxModel<MixerManager.InputMixerConfiguration>( inputMixers ) );
	...
	public void setItem( Channel item )
	{
		...
		if( hasItem() )
		{
			...
			if( config instanceof SourceConfigMixer)
			{
				...
				for( MixerManager.InputMixerConfiguration inputMixer: mSourceManager.getMixerManager().getInputMixers() )
				{
