...
public abstract class ChannelEventTracker<T extends DecodeEvent>
{
    ...
    private static final long STALE_EVENT_THRESHOLD_MS = 3000;
    ...
    /**
     * Prefixes the details the current event details text
     * @param details to prefix
     */
    public void prefixDetails(String details)
    {
        if(getEvent().getDetails() == null)
        {
            getEvent().setDetails(details);
        ...
        {
            if(!getEvent().getDetails().contains(details))
            {
                getEvent().setDetails(details + " " + getEvent().getDetails());
            }
        }
    }

    /**
     * Adds the details to the end of the existing event text or sets the event details.
     */
    public void addDetails(String details)
    {
        ...
        else
        {
            if(!getEvent().getDetails().contains(details))
            {
                getEvent().setDetails(getEvent().getDetails() + " " + details);
            }
        }
    }
