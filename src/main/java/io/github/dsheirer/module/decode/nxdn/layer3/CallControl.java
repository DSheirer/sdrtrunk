...
public abstract class CallControl extends NXDNLayer3Message
{
     ...
     */
    public CallControl(CorrectedBinaryMessage message, long timestamp)
    {
    ...
    public NXDNRadioIdentifier getSource()
    {
        ...
        {
            mSourceIdentifier = NXDNRadioIdentifier.from(getMessage().getInt(IDENTIFIER_OCTET_3));
        }
    ...
    public IntegerIdentifier getDestination()
    {
        if(mDestinationIdentifier == null)
        {
            mDestinationIdentifier = switch (getCallType())
            {
                ...
                default -> NXDNRadioIdentifier.to(getMessage().getInt(IDENTIFIER_OCTET_5));
            };
        }
