...
public class LastHeardPanel extends JPanel implements Listener<ProcessingChain>
{
    private static final long serialVersionUID = 1L;
    private final static Logger mLog = LoggerFactory.getLogger(LastHeardPanel.class);
    ...
    private LastHeardModel mLastHeardModel;
     ...
     */
    public LastHeardPanel(IconModel iconModel, UserPreferences userPreferences, AliasModel aliasModel)
    {
        ...
        mLastHeardModel = new LastHeardModel(aliasModel, iconModel);
        mUserPreferences = userPreferences;
        mTimestampCellRenderer = new TimestampCellRenderer();
        mTable = new JTable(mLastHeardModel);
    ...
    public void receive(final ProcessingChain processingChain)
    {
        ...
        {
            mCurrentEventHistory.removeListener(mLastHeardModel);
        }
        ...
        EventQueue.invokeLater(() -> {
            if(processingChain != null)
            {
                ...
                mLastHeardModel.clearAndSet(mCurrentEventHistory.getItems());
                processingChain.getDecodeEventHistory().addListener(mLastHeardModel);
                mLastSeenManagementPanel.enableButtons();
            }
            else
            {
                mCurrentEventHistory = null;
                mLastHeardModel.clearAndSet(Collections.emptyList());
    ...
    public class LastSeenManagementPanel extends JPanel
    {
        ...
        private void initializeFilterButton()
        {
            ...
            mFilterButton = new EventFilterButton<>("Last Heard To Channel Filter Editor", filterSet);
        }

        private void initializeClearButton() {
            mEventClearButton = new EventClearButton(
                    ((LastHeardModel) mTable.getModel()).getMaxMessageCount()
            );
            mEventClearButton.setEventClearHandler(new EventClearHandler() {
                @Override
                public void onHistoryLimitChanged(int newHistoryLimit) {
                    ((LastHeardModel) mTable.getModel()).setMaxMessageCount(newHistoryLimit);
                }

                @Override
                public void onClearHistoryClicked() {
                    ((LastHeardModel) mTable.getModel()).clear();
                }
            });
        }
    }
