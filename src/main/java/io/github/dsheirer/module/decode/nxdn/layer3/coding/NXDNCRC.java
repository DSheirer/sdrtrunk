...
public class NXDNCRC
{
    ...
    private static final int CRC_6_TAPS = 0x27; //Taps: (S6), S5, S2, S1 and S0

    private static final IntField CHECKSUM_SCCH = IntField.length7(25);
    private static final int CRC_7_FEEDBACK_MASK = 0x40;
    private static final int CRC_7_REGISTER_MASK = 0x7F;
    private static final int CRC_7_TAPS = 0x09; //Taps: (S7), S3 and S0
    ...
    /**
     * Performs CRC 12 check against the decoded FACCH3 message.
     * @param message to check
     * @return true if the message passes the CRC check.
     */
    public static boolean checkFACCH3(CorrectedBinaryMessage message)
    {
        return checkCRC(message, 80, CRC_12_REGISTER_MASK, CRC_12_FEEDBACK_MASK, CRC_12_TAPS, CHECKSUM_FACCH1);
    }
    ...
    /**
     * Performs CRC 7 check against the decoded SCCH Layer 3 message.
     * @param message to check
     * @return true if the message passes the CRC check.
     */
    public static boolean checkSCCH(CorrectedBinaryMessage message)
    {
        return checkCRC(message, 25, CRC_7_REGISTER_MASK, CRC_7_FEEDBACK_MASK, CRC_7_TAPS, CHECKSUM_SCCH);
    }
