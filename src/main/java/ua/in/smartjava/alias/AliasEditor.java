...
import ua.in.smartjava.alias.action.AliasActionEditor;
import ua.in.smartjava.alias.id.AliasIdentifierEditor;
			...
			implements ActionListener, Listener<AliasEvent>
{
	...
	private static final String DEFAULT_NAME = "select an ua.in.smartjava.alias";
	...
	private void init()
	{
		...
		btnSave.setToolTipText( "Save ua.in.smartjava.alias changes to the playlist" );
		btnSave.addActionListener( AliasEditor.this );
		add( btnSave );

		JButton btnReset = new JButton( "Reset" );
		btnReset.setToolTipText( "Reset ua.in.smartjava.alias changes since last save" );
	...
	{
		//If this is the currently displayed ua.in.smartjava.alias, reload it
		if( hasItem() && getItem() == event.getAlias() )
		{
			switch( event.getEvent() )
			{
				case CHANGE:
					//Alias changed - reset the editor with changed ua.in.smartjava.alias
