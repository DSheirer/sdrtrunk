 ...
 *
 ******************************************************************************/
package ua.in.smartjava.edac.trellis;

import ua.in.smartjava.bits.BinaryMessage;
import ua.in.smartjava.bits.CorrectedBinaryMessage;
...
public class ViterbiDecoder_3_4_DMR extends ViterbiDecoder
{
    ...
    /**
     * Decodes a 3/4 rate trellis coded modulation (TCM) encoded DMR binary ua.in.smartjava.message containing 196 ua.in.smartjava.bits that have
     * already been deinterleaved.
     *
     * @param encodedMessage to decode that has already been deinterleaved.
     * @return decoded ua.in.smartjava.message
     */
    ...
    /**
     * Extracts the decoded/corrected ua.in.smartjava.message from the surviving path argument.
     *
     * @param path to extract a ua.in.smartjava.message from
     * @return corrected binary ua.in.smartjava.message.
     */
    private static CorrectedBinaryMessage getMessage(Path path)
    {
        ...
        //Transfer the corrected error count to the ua.in.smartjava.message
    ...
    /**
     * Extracts (49) four-bit symbols (196 ua.in.smartjava.bits / 4) from the deinterleaved DMR binary ua.in.smartjava.message
     *
     * @param encodedMessage that deinterleaved and contains 196-ua.in.smartjava.bits of 3/4 TCM encoding
     * @return symbols
     */
    public int[] getSymbols(BinaryMessage encodedMessage)
    {
        //Ensure we have an integral number of transmitted symbols (nibbles) in the ua.in.smartjava.message
        if(encodedMessage.size() % getOutputBitLength() != 0)
        {
            throw new  IllegalArgumentException("Encoded ua.in.smartjava.message must contain an integral number of 4-bit symbols -" +
                " ua.in.smartjava.message size: " + encodedMessage.size());
        }
