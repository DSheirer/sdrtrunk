 ...
 * 
 * Passport ua.in.smartjava.message blocks are 68 ua.in.smartjava.bits in length as follows:
 *    0 -  8: Sync
 * 	  9 - 59: ua.in.smartjava.message ua.in.smartjava.bits
 *   60 - 67: CRC-7 check ua.in.smartjava.bits plus 1 parity
 *        
...
public class CRCPassport
{
	...
	/**
	 * Determines if ua.in.smartjava.message ua.in.smartjava.bits 9 - 59 pass the CRC checksum
	 * contained in ua.in.smartjava.bits 60 - 68, using a lookup table of CRC checksum values
	...
	public static CRC check( BitSet msg )
	{
        ...
        //Iterate ua.in.smartjava.bits that are set and XOR running checksum with lookup value
        for (int i = msg.nextSetBit( 9 ); i >= 9 && i <= 59; i = msg.nextSetBit( i+1 ) ) 
		{
			calculated ^= sCHECKSUMS[ i - 9 ];
		}

		//Apply the ua.in.smartjava.message checksum to derive the residual
     ...
     * are in the final 2 ua.in.smartjava.bits, owing to the forward looking soft bit detection
     * used in the decoder.  For right now, correct any messages where the 
     * residual indicates that the final 1 or 2 ua.in.smartjava.bits are bad.
     */
    ...
    /**
     * Performs an even parity check on the bitset.  If the number of ua.in.smartjava.bits that
