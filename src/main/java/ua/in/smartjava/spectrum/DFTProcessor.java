...
import ua.in.smartjava.dsp.filter.Window;
...
public class DFTProcessor implements Listener<ComplexBuffer>, IFrequencyChangeProcessor, IDFTWidthChangeProcessor
{
    ...
    //Fixed size ua.in.smartjava.sample queue.  We set the size at 12 based on the highest
    //ua.in.smartjava.sample rate tuner (10 MHz) producing 153 frames per second and the lowest
    ...
    /**
     * Sets the processor mode to Float or Complex, depending on the ua.in.smartjava.sample
    ...
    {
        //TODO: make sure frame rate & ua.in.smartjava.sample rate ua.in.smartjava.sample requirement doesn't
    ...
    /**
     * Places the ua.in.smartjava.sample into a transfer queue for future processing.
     */
    @Override
    ...
    private float[] getSamples()
    {
		...
		/* Fill the rest of the ua.in.smartjava.buffer with new samples */
    ...
    private void purge(int samplesToPurge)
    {
        if(samplesToPurge <= 0)
        {
            throw new IllegalArgumentException("DFTProcessor - cannot purge "
                + "negative ua.in.smartjava.sample amount");
        }
