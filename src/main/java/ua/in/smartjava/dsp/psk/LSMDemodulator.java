...
import ua.in.smartjava.instrument.tap.stream.EyeDiagramData;
...
import ua.in.smartjava.buffer.FloatAveragingBuffer;
import ua.in.smartjava.dsp.filter.interpolator.RealInterpolator;
...
//			}
	...
	public class GardnerSymbolTiming implements ComplexSampleListener
	{
		...
		public void receive( float inphase, float quadrature )
		{
			...
			/* Mix incoming ua.in.smartjava.sample with costas loop to remove any rotation
			...
			if( mMu <= 1.0f )
			{
				...
				/* Calculate interpolated middle ua.in.smartjava.sample and current ua.in.smartjava.sample */
				...
				/* Multiply current ua.in.smartjava.sample and conjugate (negated quadrature) of
				 * previous ua.in.smartjava.sample to get symbols to use for gardner error feedback */
				...
				/* Adjust ua.in.smartjava.sample timing based on error of current ua.in.smartjava.sample */
...
//						ComplexSample.multiply( currentSymbol, POINT_45_DEGREES ) );
	 ...
	 * frequency adjustments to the tuner ua.in.smartjava.channel ua.in.smartjava.source.
	 * 
