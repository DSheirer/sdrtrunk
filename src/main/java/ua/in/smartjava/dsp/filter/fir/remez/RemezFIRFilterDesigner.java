package ua.in.smartjava.dsp.filter.fir.remez;
...
public class RemezFIRFilterDesigner
{
	...
	/**
	 * Linear FIR ua.in.smartjava.filter designer using the Parks-McClelland Remez exchange algorithm.
	 * 
	 * Construct a ua.in.smartjava.filter specification using the builder methods in the FIRFilterSpecification
	 * class and construct an instance of this class with the specification.  Check the isValid()
	 * method after construction to see if the ua.in.smartjava.filter design was successful.  If successful, use the
	 * getFrequencyResponse() and getImpulseResponse() methods to access the ua.in.smartjava.filter taps as the
	 * specified length.
	 * 
	 * If the ua.in.smartjava.filter cannot be designed from the specification the isValid() method will return
	...
	/**
	 * Designs the ua.in.smartjava.filter according to the specification
	 */
	private void design()
	{
    	...
    	catch( FilterDesignException fde )
    	{
    		mLog.error( "Filter design error - couldn't find extremal indices at count [" + 
    				iterationCount + "] try changing ua.in.smartjava.filter order up/down from [" +
	...
	/**
	 * Indicates if the ua.in.smartjava.filter was successfully created from the ua.in.smartjava.filter specification.  Check this
	 * method before accessing frequency response or ua.in.smartjava.filter response, otherwise a FilterDesignException
	 * is thrown.
	 * 
	 * @return true if the ua.in.smartjava.filter was successfully designed.
	 */
	...
	/**
	 * Frequency response of the designed ua.in.smartjava.filter with double precision
	 * 
	 * @return half ( length / 2 ) of ua.in.smartjava.filter frequency response sampled at 0.0 - PI radians
	 * @throws FilterDesignException if the specified ua.in.smartjava.filter cannot be designed
	 */
	public double[] getFrequencyResponse() throws FilterDesignException
	{
		if( !mConverged )
		{
			throw new FilterDesignException( "Can't create ua.in.smartjava.filter from specification - failed "
					+ "to converge" );
		}
		
		//Resample the final polynomial at the desired ua.in.smartjava.filter length
    	double[] resampled = resample();

    	//Apply Parks/McClelland frequency response corrections according to the ua.in.smartjava.filter type
    	return correctFrequencyResponse( resampled );
	}

	/**
	 * Impulse response of the designed ua.in.smartjava.filter with floating point precision.
	 * 
	 * @return ua.in.smartjava.filter impulse response
	 * @throws FilterDesignException if the specified ua.in.smartjava.filter cannot be designed
	 */
    ...
    /**
     * Impulse response of the designed ua.in.smartjava.filter with double precision
     */
    ...
    /**
     * Calculates the frequency response of this ua.in.smartjava.filter at the specified cosine of the frequency
     * 
     ...
     * the ua.in.smartjava.filter specification by comparing the maximum absolute error value against the delta
    ...
    /**
     * Resamples the designed ua.in.smartjava.filter at half of the ua.in.smartjava.filter length at evenly spaced intervals of the
     * frequency ua.in.smartjava.spectrum from 0 to PI radians.
     * 
     * @return frequency response of the ua.in.smartjava.filter sampled the desired ua.in.smartjava.filter length and order
     */
    ...
    /**
     * Applies Parks/McClelland ua.in.smartjava.filter type correction to the final frequency response array
     * 
     * @param frequencyResponse of the resampled ua.in.smartjava.filter design from 0 <> ~ PI radians
     * 
