package ua.in.smartjava.dsp.filter.equalizer;
...
public class CMAEqualizer implements Listener<Complex>
{
	...
	public CMAEqualizer( float modulus, float mu )
	{
		 ...
		 * This means that the output is solely determined by the last ua.in.smartjava.sample
		 * entering the ua.in.smartjava.filter, initially. */
	...
	{
		/* Calculate the error.  The ideal ua.in.smartjava.sample magnitude is unity (1.0).
		 * When the ua.in.smartjava.sample is not at unity, the error vector points in the
		 * opposite direction with a magnitude equal to 1 - ua.in.smartjava.sample magnitude.*/
		...
		{
			// tap -= mu * conj(ua.in.smartjava.sample) * error
