...
public class FloatFIRFilter implements RealSampleListener
{
	...
	public FloatFIRFilter( float[] coefficients, float gain )
	{
		...
		//Fill the ua.in.smartjava.buffer with zero valued samples, so we don't have to check for null
	...
	/**
	 * Dispatch the filtered ua.in.smartjava.sample to all registered listeners
	 */
	...
	private void generateIndexMap( int size )
	{
		...
		//For each subsequent ua.in.smartjava.map row, increment the value of the preceding row
