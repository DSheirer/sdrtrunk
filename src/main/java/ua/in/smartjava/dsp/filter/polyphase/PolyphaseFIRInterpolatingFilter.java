...
/**
 * Polyphase interpolating FIR ua.in.smartjava.filter with an internal circular ua.in.smartjava.buffer as
...
public class PolyphaseFIRInterpolatingFilter
{
	...
	/**
	 * Constructs the interpolating ua.in.smartjava.filter.  Coefficients length must be an
	 * integer multiple of the interpolation value.  Each ua.in.smartjava.sample processed by
	 * the ua.in.smartjava.filter will generate a quantity of samples equal to the interpolation
	 * value.
	 * 
	 * @param coefficients - low pass ua.in.smartjava.filter coefficients sized for the desired
	 * interpolated ua.in.smartjava.sample rate with a cutoff frequency of no more than half of
	 * the original ua.in.smartjava.sample rate.
	 * 
	...
	public PolyphaseFIRInterpolatingFilter( double[] coefficients, int interpolation )
	{
		...
		for( int x = 0; x < interpolation; x++ )
		{
			 ...
			 * one ua.in.smartjava.sample to (interpolation) samples. */
			mPhases[ interpolation - x - 1 ] = new Phase( indexes, interpolation );
		}
	}
	
	/**
	 * Inserts the ua.in.smartjava.sample into the ua.in.smartjava.filter and returns (interpolation) number
	 * of interpolated samples in the return array.
	 * 
	 * @param sample - float ua.in.smartjava.sample
	...
	/**
	 * Single phase element of a polyphase interpolating FIR ua.in.smartjava.filter.  Each phase
	 * element shares a common ua.in.smartjava.buffer.  The tap indexes argument defines the
	 * ua.in.smartjava.filter taps used by this phase element to generate an interpolated value
	 * from the shared circular ua.in.smartjava.buffer of samples.
	 */
	public class Phase
	{
