 ...
 *
 *	   Root Raised Cosine ua.in.smartjava.filter designer:
...
public class FilterFactory
{
    ...
    /**
     * Generates coefficients for a unity-gain, windowed low-pass ua.in.smartjava.filter
     *
     * @param sampleRate - hertz
     * @param frequency - cutoff in hertz
     * @param length - ua.in.smartjava.filter length
    ...
    /**
     * Normalizes all ua.in.smartjava.filter coefficients to achieve unity (1) gain, by ensuring
     ...
     * the frequency response of the ua.in.smartjava.filter.  Used in converting a low-pass
     * ua.in.smartjava.filter to a high-pass ua.in.smartjava.filter.
     *
    ...
    /**
     * Generates ua.in.smartjava.filter coefficients for a unity-gain, odd-length, windowed,
     * low pass ua.in.smartjava.filter with passband from 0-hertz to the cutoff frequency.
     *
     * @param sampleRate - hertz
     * @param cutoff - frequency in hertz
     * @param filterLength - odd ua.in.smartjava.filter length
    ...
    /**
     * Creates a low-pass ua.in.smartjava.filter with ~ 0.1 db ripple in the pass band
     *
     * Note: stop frequency - pass frequency defines the transition band.
     */
    /**
     * Creates a low-pass ua.in.smartjava.filter with ~0.1 db ripple in the pass band.  The
                                     ...
                                     boolean forceOddLength)
    {
        if(stopFrequency < passFrequency || stopFrequency > (sampleRate / 2))
        {
            throw new IllegalArgumentException("FilterFactory - low pass "
                + "ua.in.smartjava.filter pass frequency [" + passFrequency + "] must be "
                + "less than the stop frequency [" + stopFrequency + "] "
                + "and must be less than half [" + (int) (sampleRate / 2) +
                "] of the ua.in.smartjava.sample rate [" + sampleRate + "]");
        }
    ...
    /**
     * Generates ua.in.smartjava.filter coefficients for a unity-gain, odd-length, windowed,
     * high pass ua.in.smartjava.filter with passband from cutoff frequency to half the ua.in.smartjava.sample
     * rate.
     *
     * @param sampleRate - hertz
     * @param cutoff - frequency in hertz
                                      ...
                                      WindowType windowType)
    {
        ...
        //equivalent, so that when we generate the low pass ua.in.smartjava.filter, prior to
    ...
    /**
     * Determines the number of fir ua.in.smartjava.filter taps required to produce the
    ...
    /**
     * Determines decimation rate(s) for a polyphase decimation ua.in.smartjava.filter.
     *
     * @param sampleRate - starting ua.in.smartjava.sample rate
     * @param decimatedRate - final (decimated) output rate
     * @return - set of integer decimation rates for a single or multi-stage
     * polyphase ua.in.smartjava.filter decimation chain
     * @throws - AssertionException if ua.in.smartjava.sample rate is not a multiple of 48 kHz
     */
                                                    ...
                                                    long stopFrequency)
    {
        ...
        if(sampleRate % decimatedRate != 0)
        {
            throw new IllegalArgumentException("Decimated rate must be an "
                + "integer multiple of ua.in.smartjava.sample rate");
        }

        int decimation = (int) (sampleRate / decimatedRate);

        //Decimation rates below 20 will use single stage polyphase ua.in.smartjava.filter
        if(decimation < 20)
        {
            rates = new int[1];
            rates[0] = decimation;
...
//					  "]");
     ...
     * poly-phase decimation ua.in.smartjava.filter chain, to produce a final ua.in.smartjava.sample rate of
...
//				  "]" );
    ...
    /**
     * Assumes that the pass band is 1/4 of the output ua.in.smartjava.sample rate.
     *
...
//		coefficients = Window.apply( window, coefficients );
    ...
    /**
     * Creates root raised cosine ua.in.smartjava.filter coefficients with a tap count equal
     * to the symbols x samplesPerSymbol + 1.
     *
     * Symbol count should be an even integer.
     *
     * Ported to java from gnuradio/ua.in.smartjava.filter/firdes.cc
     *
     ...
     * @return - ua.in.smartjava.filter coefficients
     */
    ...
    /**
     * Creates a ua.in.smartjava.filter from the ua.in.smartjava.filter specification
     *
     * @param specification
     * @return
     * @throws FilterDesignException if the ua.in.smartjava.filter cannot be designed
     */
