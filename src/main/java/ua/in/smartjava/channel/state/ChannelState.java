...
import ua.in.smartjava.channel.metadata.MutableMetadata;
...
import static ua.in.smartjava.source.tuner.frequency.FrequencyChangeEvent.Event.NOTIFICATION_FREQUENCY_CHANGE;
    ...
    IAttributeChangeRequestListener, IOverflowListener
{
    ...
    /**
     * Channel state tracks the overall state of all processing modules and decoders configured for the ua.in.smartjava.channel and
     * provides squelch control and decoder state reset events.
     *
     * Uses a state enumeration that defines allowable ua.in.smartjava.channel state transitions in order to track a call or data decode
     ...
     *     signalling updates have been received, and the fade timer has expired.  This phase allows for ua.in.smartjava.gui updates to
     *     signal to the user that the call is ended, while continuing to display the call details for the user
     * TEARDOWN:  Indicates a traffic ua.in.smartjava.channel that will be torn down for reuse.
     */
    ...
    /**
     * Metadata for this ua.in.smartjava.channel containing ua.in.smartjava.channel details, primary and secondary entities for call events and any
     * decoded messages.  This metadata is primarily intended to be used by graphical components to convey the
     * details of the current ua.in.smartjava.channel state and to provide metadata backing any decoded ua.in.smartjava.audio packets produced by
     * any ua.in.smartjava.audio streams for this ua.in.smartjava.channel.
     */
    ...
    /**
     * Resets this ua.in.smartjava.channel state and prepares it for reuse.
     */
    @Override
    public void reset()
    {
    ...
    /**
     * Sets the ua.in.smartjava.channel state to the specified state, or updates the timeout values so that the state monitor will not
     * change state.  Broadcasts a squelch event when the state changes and the ua.in.smartjava.audio squelch state should change.  Also
     * broadcasts changed attribute and decoder state events so that external processes can maintain sync with this
     * ua.in.smartjava.channel state.
     */
    ...
    /**
     * This method is invoked if the ua.in.smartjava.source ua.in.smartjava.buffer provider goes into overflow state.  Since this is an external state,
    ...
    /**
     * Broadcasts a ua.in.smartjava.channel state event to any registered listeners
     */
    ...
    /**
     * Registers a listener to be notified when a traffic ua.in.smartjava.channel call event is
    ...
    public void configureAsTrafficChannel(TrafficChannelManager manager, CallEvent callEvent)
    {
		...
		/* Broadcast the call event details as metadata for the ua.in.smartjava.audio manager */
     ...
     * functions.
     *
     * Monitors decoder state events to automatically transition the ua.in.smartjava.channel state to IDLE (standard ua.in.smartjava.channel) or to
     * TEARDOWN (traffic ua.in.smartjava.channel) when decoding stops or the monitored ua.in.smartjava.channel returns to a no signal state.
     *
