...
public class Trellis_3_4_Rate
{
	...
	/* Constellation and state tribit lookup ua.in.smartjava.map to find the input tribit */
	...
	/**
	 * Creates a lookup ua.in.smartjava.map for state to input tribit values for a given
	...
	/**
	 * Decodes a 196-bit 3/4 rate convolutional encoded ua.in.smartjava.message located between
	 * start and end indexes and returns the decoded 144-bit ua.in.smartjava.message overlayed
	 * upon the original ua.in.smartjava.message with the remaining 52 ua.in.smartjava.bits cleared to zero.
	 * 
	 * @return - original ua.in.smartjava.message with decoded ua.in.smartjava.message ua.in.smartjava.bits..
	 */
	public boolean decode( BinaryMessage message, int start, int end )
	{
		...
		{
			/* Clear the original ua.in.smartjava.message ua.in.smartjava.bits */
			message.clear( start, end );

			List<Node> nodes = path.getNodes();
			
			/* Load each of the nodes' state tribit values into the original ua.in.smartjava.message */
			for( int x = 1; x < 50; x++ )
			{
	...
	/**
	 * Path metrics maintains a ua.in.smartjava.map of the current best path metrics that end
	...
	{
		String raw = "12:58:51.635 DEBUG ua.in.smartjava.dsp.fsk.P25MessageFramer - AFTER  DEINTERLEAVE: ";
