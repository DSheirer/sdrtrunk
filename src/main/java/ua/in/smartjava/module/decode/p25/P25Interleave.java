...
public class P25Interleave
{
	...
	private static int[] VOICE_DEINTERLEAVE = new int[] { 0,7,12,19,24,31,36,43,
		...
		118,125,130,137,142 };
	/**
	 * Deinterleaves the 196-bit block in ua.in.smartjava.message, identified by start and end
	 * bit positions.  Note: end index (exclusive) should be one more than the 
	 * last bit in the block.
	 * 
	 * @param message - ua.in.smartjava.source ua.in.smartjava.message to deinterleave
			...
			int start, int end )
	{
		...
		/* Clear block ua.in.smartjava.bits in ua.in.smartjava.source ua.in.smartjava.message */
		message.clear( start, end );

		/* Iterate only the set ua.in.smartjava.bits in the original ua.in.smartjava.message and apply
		 * the deinterleave -- we don't have to evaluate the 0 ua.in.smartjava.bits */
    ...
    /**
     * Interleaves the 196-bit block in ua.in.smartjava.message, identified by start and end
     * bit positions.  Note: end index (exclusive) should be one more than the 
     * last bit in the block.
     * 
     * @param message - ua.in.smartjava.source ua.in.smartjava.message to interleave
     * @param start - starting bit index for the block
     * @param end - ending bit index for the block, plus 1
     */
