...
import ua.in.smartjava.module.decode.p25.message.ldu.LDU2Message;
import ua.in.smartjava.module.decode.p25.message.ldu.lc.LDULCMessageFactory;
import ua.in.smartjava.module.decode.p25.message.pdu.PDUMessage;
import ua.in.smartjava.module.decode.p25.message.pdu.PDUMessageFactory;
import ua.in.smartjava.module.decode.p25.message.pdu.confirmed.PDUConfirmedMessage;
import ua.in.smartjava.module.decode.p25.message.pdu.confirmed.PDUConfirmedMessageFactory;
import ua.in.smartjava.module.decode.p25.message.tdu.TDUMessage;
import ua.in.smartjava.module.decode.p25.message.tdu.lc.TDULCMessageFactory;
...
import ua.in.smartjava.module.decode.p25.message.tsbk.TSBKMessageFactory;
import ua.in.smartjava.module.decode.p25.message.vselp.VSELP1Message;
import ua.in.smartjava.module.decode.p25.message.vselp.VSELP2Message;
...
import ua.in.smartjava.bits.BitSetFullException;
import ua.in.smartjava.bits.ISyncDetectListener;
import ua.in.smartjava.bits.MultiSyncPatternMatcher;
import ua.in.smartjava.bits.SoftSyncDetector;
import ua.in.smartjava.bits.SyncDetector;
import ua.in.smartjava.dsp.psk.LSMDemodulator;
import ua.in.smartjava.dsp.symbol.Dibit;
import ua.in.smartjava.dsp.symbol.FrameSync;
import ua.in.smartjava.edac.BCH_63_16_11;
...
public class P25MessageFramer implements Listener<Dibit>
{
	...
	/**
	 * Constructs a P25 ua.in.smartjava.message framer to receive a stream of symbols and
	 * detect the sync pattern then capture the following stream of symbols up
	 * to the ua.in.smartjava.message length, and then broadcast that bit ua.in.smartjava.buffer to the registered
	 * listener.
	 */
	public P25MessageFramer( AliasList aliasList )
	{
		...
		/**
		 * We use two ua.in.smartjava.message assemblers to catch any sync detections, so that
		 * if we have a false trigger then we still have a chance to catch the 
		 * valid ua.in.smartjava.message. If we have two false triggers before the arrival of
		 * the actual ua.in.smartjava.message sync, then the third or subsequent real or false
		 * sync pattern will produce a debug ua.in.smartjava.message indicating no assemblers
		 * are available..
		 */
    ...
    private class P25MessageAssembler
    {
    	 ...
    	 * account for the 48-bit sync pattern which is not included in ua.in.smartjava.message */
        ...
        public void receive( Dibit dibit )
        {
        	if( mActive )
        	{
        		...
        		else
        		{
                    ...
                    /* Check the ua.in.smartjava.message for complete */
                    if( mMessage.isFull() )
                    {
                    	checkComplete();
                    }
        ...
        private void checkComplete()
        {
        	switch( mDUID )
        	{
				...
				case LDU1:
					...
					/* Convert the LDU1 ua.in.smartjava.message into a link control LDU1 ua.in.smartjava.message */
				...
				case PDUC:
					...
					if( mThreeQuarterRate.decode( mMessage, 
							mMessage.size() - 196, mMessage.size() ) )
					{
						/* Resize the ua.in.smartjava.message and adjust the ua.in.smartjava.message pointer
						 * to account for removing 48 + 4 parity ua.in.smartjava.bits */
				...
				case TDULC:
					...
					/* Convert to an appropriate link control ua.in.smartjava.message */
     ...
     * patterns and apply immediate phase correction so that ua.in.smartjava.message processing
     * can continue. 
     */
    	...
    	public CostasPhaseErrorDetector( FrameSync frameSync, LSMDemodulator demodulator, double correction )
    	{
    		...
    		setListener( new ISyncDetectListener()
			{
				...
				public void syncDetected()
				{
					...
					/* Since we detected a sync pattern, start a ua.in.smartjava.message assembler */
		        	for( P25MessageAssembler assembler: mAssemblers )
		        	{
		        		if( !assembler.isActive() )
		        		{
