...
public class MDCMessageProcessor implements Listener<BinaryMessage>
{
    ...
    {
		/**
		 * De-interleave the 112 ua.in.smartjava.bits of ua.in.smartjava.message one, starting after the sync
		 * pattern
		 */
		deinterleave( buffer, 40 );

		/**
		 * For simplicity, we also deinterleave ua.in.smartjava.message 2, which may or may not
		 * be there, starting after the initial sync ( 0 - 39 ) and the initial
		 * ua.in.smartjava.message ( 40 - 151 ) and the second sync ( 152 - 191 ).
		 */
		deinterleave( buffer, 192 );

		/**
		 * Wrap the ua.in.smartjava.buffer in a ua.in.smartjava.message along with the designated ua.in.smartjava.alias list
	...
	/**
	 * Removes NRZ-I encoding from the entire ua.in.smartjava.buffer.  Assumes that the initial
	 * bit will be a 0 after decode, despite what is currently in the ua.in.smartjava.buffer,
	 * in order to maintain correct polarity of the decoded ua.in.smartjava.message.
	 */
	...
	/**
	 * Deinterleaves a 112-bit packet, starting at the offset into the ua.in.smartjava.buffer
	 */
	private void deinterleave( BinaryMessage buffer, int offset )
	{
		if( buffer.size() < sMESSAGE_LENGTH + offset )
		{
			throw new IllegalArgumentException( "MDCMessageProcessor - "
				+ "cannot deinterleave ua.in.smartjava.message - ua.in.smartjava.message ua.in.smartjava.buffer too short" );
		}
		...
		//Overlay the deinterleaved ua.in.smartjava.bits back onto the ua.in.smartjava.source ua.in.smartjava.message and
		//return it
