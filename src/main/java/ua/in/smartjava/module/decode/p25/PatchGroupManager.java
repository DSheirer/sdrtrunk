...
import ua.in.smartjava.alias.PatchGroupAlias;
...
public class PatchGroupManager
{
     ...
     * ua.in.smartjava.message commanding commanding a patch group delete.
     */
    private void removePatchGroup(String patchGroupID, boolean expired)
    {
        ...
        if(patchGroup != null)
        {
            //Cleanup the ua.in.smartjava.alias list
            if(mAliasList != null)
            {
                ...
                if(alias instanceof PatchGroupAlias)
                {
                    ...
                    //Replace our temporary patch group ua.in.smartjava.alias with the original ua.in.smartjava.alias for the patch group ID
    ...
    /**
     * Adds/updates a patch group ua.in.smartjava.alias to the ua.in.smartjava.alias list containing aliases for each of the patched talkgroups.  If the
     * patch group ua.in.smartjava.alias already exists, any patched talkgroups will be removed from the existing patch group ua.in.smartjava.alias and
    ...
    public void updatePatchGroup(String patchGroupID, List<String> patchedTalkgroups)
    {
        ...
        //If we make any changes to the patch group, update the patch group ua.in.smartjava.alias
        if(updated)
        {
            PatchGroupAlias patchGroupAlias = null;

            //Check for an existing ua.in.smartjava.alias for the patch talkgroup - do not include wildcard aliases
            if(mAliasList != null)
            {
