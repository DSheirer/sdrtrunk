		...
		if( mMessageListener != null )
		{
			...
			else
			{
				...
				if( message.isValid() )
				{
					 ...
					 * the ESN high ua.in.smartjava.message after 2 messages, so that it doesn't get
					 * paired with the wrong esn low ua.in.smartjava.message
					 */
					if( message.getMessageType() == MessageType.ID_ESNH )
					{
						...
						{
							/* Check that the stored ua.in.smartjava.message is not older than
							 * 5 seconds */
					...
					else if( message.getMessageType() == MessageType.ID_ESNL )
					{
						...
						{
							/* Check that the stored ua.in.smartjava.message is not older than 5 seconds */
