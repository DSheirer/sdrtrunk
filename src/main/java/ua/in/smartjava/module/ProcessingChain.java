 ...
 *
 ******************************************************************************/
package ua.in.smartjava.module;

import ua.in.smartjava.audio.AudioPacket;
import ua.in.smartjava.audio.IAudioPacketListener;
import ua.in.smartjava.audio.IAudioPacketProvider;
import ua.in.smartjava.audio.squelch.ISquelchStateListener;
import ua.in.smartjava.audio.squelch.ISquelchStateProvider;
import ua.in.smartjava.audio.squelch.SquelchState;
import ua.in.smartjava.channel.heartbeat.Heartbeat;
import ua.in.smartjava.channel.heartbeat.IHeartbeatListener;
import ua.in.smartjava.channel.heartbeat.IHeartbeatProvider;
import ua.in.smartjava.channel.metadata.AttributeChangeRequest;
import ua.in.smartjava.channel.metadata.IAttributeChangeRequestListener;
import ua.in.smartjava.channel.metadata.IAttributeChangeRequestProvider;
import ua.in.smartjava.channel.state.ChannelState;
...
import ua.in.smartjava.channel.state.IDecoderStateEventProvider;
import ua.in.smartjava.controller.channel.Channel.ChannelType;
import ua.in.smartjava.controller.channel.ChannelEvent;
import ua.in.smartjava.controller.channel.IChannelEventListener;
import ua.in.smartjava.controller.channel.IChannelEventProvider;
import ua.in.smartjava.message.IMessageListener;
import ua.in.smartjava.message.IMessageProvider;
import ua.in.smartjava.message.Message;
import ua.in.smartjava.module.decode.event.CallEvent;
import ua.in.smartjava.module.decode.event.CallEventModel;
import ua.in.smartjava.module.decode.event.ICallEventListener;
...
import ua.in.smartjava.module.log.EventLogger;
...
import ua.in.smartjava.sample.real.IUnFilteredRealBufferProvider;
...
/**
 * Processing chain provides a framework for connecting a complex or real ua.in.smartjava.sample
 * ua.in.smartjava.source to a set of one primary decoder and zero or more auxiliary decoders.
 ...
 * listeners to receive aggregated messages, call events, and ua.in.smartjava.audio packets.
 *
 * Normal setup sequence:
 *
 * 1) Add one or more modules
 * 2) Register listeners to receive messages, call events, ua.in.smartjava.audio, etc.
 * 3) Add a valid ua.in.smartjava.source
 * 4) Invoke the start() method to start processing.
 * 5) Invoke the stop() method to stop processing.
 *
 * Optional: if you want to reuse the processing chain with a new ua.in.smartjava.sample ua.in.smartjava.source,
...
public class ProcessingChain implements IChannelEventListener
{
     ...
     * a ua.in.smartjava.source and sending those samples to the decoders.
     */
    ...
    /**
     * Indicates if this chain currently has a valid ua.in.smartjava.sample ua.in.smartjava.source.
     */
    ...
    /**
     * Applies a ua.in.smartjava.sample ua.in.smartjava.source to this processing chain.  Processing won't
     * start until the start() method is invoked.
     *
     * @param source - real or complex ua.in.smartjava.sample ua.in.smartjava.source
     * @throws IllegalStateException if the processing chain is currently
     *                               processing with another ua.in.smartjava.source.  Invoke stop() before applying a new
     *                               ua.in.smartjava.source.
     */
    public void setSource(Source source) throws IllegalStateException
    {
        if(isProcessing())
        {
            throw new IllegalStateException("Processing chain is currently processing.  Invoke stop() on the " +
                "processing chain before applying a new ua.in.smartjava.sample ua.in.smartjava.source");
        }
    ...
    /**
     * Adds a ua.in.smartjava.module to the processing chain.  Each ua.in.smartjava.module is tested for the
     ...
     * and events that are produced by any ua.in.smartjava.module are automatically routed to
     * all other components that support the corresponding listener interface.
     *
     * At least one ua.in.smartjava.module should consume complex samples and either produce
     * decoded messages and/or ua.in.smartjava.audio, or produce decoded real ua.in.smartjava.sample buffers
     * for all other modules to consume.
     *
     * @param module - processing ua.in.smartjava.module, demodulator, decoder, ua.in.smartjava.source, state
     * machine, etc.
     */
    ...
    /**
     * Removes the ua.in.smartjava.module from the processing chain and deregisters the ua.in.smartjava.module as a listener and
     * as a provider
     */
    ...
    /**
     * Registers the ua.in.smartjava.module as a listener to each of the broadcasters that
     * provide the data interface(s) supported by the ua.in.smartjava.module.
     */
    ...
    /**
     * Registers the broadcaster(s) as listeners to the ua.in.smartjava.module for each
     * provider interface that is supported by the ua.in.smartjava.module.
     */
    ...
    /**
     * Unregisters the broadcaster(s) as listeners to the ua.in.smartjava.module for each
     * provider interface that is supported by the ua.in.smartjava.module.
     */
    ...
    /**
     * Starts processing if the chain has a valid ua.in.smartjava.source.  Invocations on an
    ...
    public void start()
    {
        if(mRunning.compareAndSet(false, true))
        {
            if(mSource != null)
            {
                ...
                //Setup the ua.in.smartjava.channel state to monitor ua.in.smartjava.source overflow conditions
                mSource.setOverflowListener(mChannelState);

				/* Register with the ua.in.smartjava.source to receive ua.in.smartjava.sample data.  Setup a
				 * timer task to process the ua.in.smartjava.buffer queues 50 times a second
				 * (every 20 ms) */
                switch(mSource.getSampleType())
                {
                    ...
                    case REAL:
                        ...
                        break;
                    default:
                        throw new IllegalArgumentException("Unrecognized ua.in.smartjava.source "
                            + "ua.in.smartjava.sample type - cannot start processing chain");
                }
                ...
                for(Module module : mModules)
                {
                    ...
                    {
                        mLog.error("Error starting ua.in.smartjava.module", e);
                    }
                }
            }
            else
    ...
    public void stop()
    {
        if(mRunning.compareAndSet(true, false))
        {
            ...
            if(mSource != null)
            {
				...
				/* Release the ua.in.smartjava.source */
                mSource.dispose();
                mSource = null;
            }
    ...
    /**
     * Adds the listener to receive ua.in.smartjava.audio packets from all modules.
     */
