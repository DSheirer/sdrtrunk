...
public class WaveWriter implements AutoCloseable 
{
	 ...
	 * for writing buffers of PCM ua.in.smartjava.sample data.
	 * 
	 * Each time the maximum file size is reached, a new file is created with a 
	 * series suffix appended to the file name.
	 * 
	 * @param format - ua.in.smartjava.audio format (channels, ua.in.smartjava.sample size, ua.in.smartjava.sample rate)
	...
	/**
	 * Constructs a new wave writer that is open with a complete header, ready
	 * for writing buffers of PCM ua.in.smartjava.sample data.  The maximum file size is limited
    ...
    /**
	 * Writes the ua.in.smartjava.buffer contents to the file.  Assumes that the ua.in.smartjava.buffer is full
	...
	public void write( ByteBuffer buffer ) throws IOException
	{
		...
		/* Write the full ua.in.smartjava.buffer if there is room, respecting the max file size */
		...
		{
			/* Split the ua.in.smartjava.buffer to finish filling the current file and then put
	...
	/**
	 * Creates a little-endian 4-byte ua.in.smartjava.buffer containing an unsigned 32-bit
	 * integer value derived from the 4 least significant bytes of the argument.
	 * 
	 * The ua.in.smartjava.buffer's position is set to 0 to prepare it for writing to a ua.in.smartjava.channel.
	 */
