...
public class MonoWaveReader implements AutoCloseable
{
	...
	private void open() throws IOException
	{
        ...
        if(channels != 1)
        {
            throw new IOException("Unsupported ua.in.smartjava.channel count - mono ua.in.smartjava.audio only");
        }
        ...
        //Get ua.in.smartjava.bits per ua.in.smartjava.sample
        short bitsPerSample = shortBuffer.get();
        if(bitsPerSample != 16)
        {
            throw new IOException("PCM ua.in.smartjava.sample size not supported - expecting 16 ua.in.smartjava.bits per ua.in.smartjava.sample");
        }
	...
	public static void main(String[] args)
    {
        ...
        try
        {
            ...
            reader.setListener(new Listener<RealBuffer>()
            {
                ...
                {
                    mLog.debug("Received ua.in.smartjava.buffer");
                }
            });
            reader.read();
        }
