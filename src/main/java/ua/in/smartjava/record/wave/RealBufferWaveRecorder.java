 ...
 *
 ******************************************************************************/
package ua.in.smartjava.record.wave;
...
/**
 * WAVE ua.in.smartjava.audio recorder ua.in.smartjava.module for recording real ua.in.smartjava.sample buffers to a wave file
 */
    ...
    implements IFilteredRealBufferListener, Listener<RealBuffer>
{
    ...
    /**
     * Timestamp of when the latest ua.in.smartjava.buffer was received by this recorder
     */
    ...
    public void start(ScheduledExecutorService executor)
    {
        if(mRunning.compareAndSet(false, true))
        {
            ...
            {
                mLog.error("Error starting real ua.in.smartjava.buffer recorder", io);
            }
        }
    }
    ...
    public void stop()
    {
        if(mRunning.compareAndSet(true, false))
        {
            ...
            {
                mLog.error("Error writing final ua.in.smartjava.audio buffers to recording during shutdown", ioe);
            }
        }
    }
    ...
    public void receive(RealBuffer buffer)
    {
        if(mRunning.get())
        {
            ...
            if(!success)
            {
                mLog.error("recorder ua.in.smartjava.buffer overflow - purging [" + mFile.toFile().getAbsolutePath() + "]");
                mBuffers.clear();
            }
    ...
    /**
     * Writes all ua.in.smartjava.audio currently in the queue to the file
     * @throws IOException if there are any errors writing the ua.in.smartjava.audio
     */
    ...
    public class BufferProcessor implements Runnable
    {
        public void run()
        {
            ...
            {
				/* Stop this ua.in.smartjava.module if/when we get an IO exception */
                mBuffers.clear();
                stop();
