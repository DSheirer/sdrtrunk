...
public abstract class AudioRecorder extends Module implements Listener<AudioPacket>, IAudioPacketListener
{
    ...
    /**
     * Abstract ua.in.smartjava.audio recorder that implements ua.in.smartjava.audio packet queueing and threaded ua.in.smartjava.audio conversion/writing to a file
     *
    ...
    /**
     * Path for the ua.in.smartjava.audio recording file
     */
    public Path getPath()
    {
        return mPath;
    ...
    /**
     * Latest ua.in.smartjava.audio metadata received for this recording
     */
    ...
    /**
     * Timestamp of the last ua.in.smartjava.buffer received by this recorder - allows this recorder to be monitored for automatic
    ...
    {
        //Assumes ua.in.smartjava.audio ua.in.smartjava.sample rate of 8000 samples/second or 8 samples/milli-second
     ...
     * parent ua.in.smartjava.module class.
     */
    @Override
    ...
    /**
     * Processes the ua.in.smartjava.audio packet and captures the latest Metadata for the recording for easy access.
     */
    @Override
    public void receive(AudioPacket audioPacket)
    {
        if(mRunning.get())
        {
            ...
            if(!success)
            {
                mLog.error("recorder ua.in.smartjava.buffer overflow - stopping recorder [" + getPath().toString() + "]");
                stop();
            }
        }
    }
     ...
     * ua.in.smartjava.audio recorder is closed.  There is potential for the calling thread (here) and the ua.in.smartjava.buffer processor thread to
    ...
    /**
     * Records the list of ua.in.smartjava.audio packets in the sub-class specific ua.in.smartjava.audio format.
     */
     ...
     *
     * @param executor to use in scheduling ua.in.smartjava.audio conversion and file writes.
     */
    public void start(ScheduledExecutorService executor)
    {
        if(mRunning.compareAndSet(false, true))
        {
            ...
            {
                mLog.error("Error starting ua.in.smartjava.audio recorder [" + getPath().toString() + "]", io);
    ...
    /**
     * Processes the ua.in.smartjava.audio packet queue.
     */
    private void processAudioPacketQueue()
    {
        ...
        if(!mPacketsToProcess.isEmpty())
        {
            ...
            {
                mLog.debug("Error while recording ua.in.smartjava.audio to [" + getPath().toString() + "] - stopping recorder");
                stop();
            }
    ...
    /**
     * Disposes this ua.in.smartjava.audio recorder and prepares it for reclamation
     */
    @Override
    public void dispose()
    {
    ...
    /**
     * Flushes any remaining ua.in.smartjava.audio to the output file.  This method should be implemented by subclasses where an ua.in.smartjava.audio
     * converter may contain residual frame data that should be flushed to disk before closing the ua.in.smartjava.audio file.
     */
    ...
    /**
     * Drains the ua.in.smartjava.audio packet queue and records the ua.in.smartjava.audio packets to file
     */
    public class BufferProcessor implements Runnable
    {
        ...
        public void run()
        {
            if(mProcessing.compareAndSet(false, true))
            {
                ...
                if(!mRunning.get())
                {
                    //Allow sub-classes to flush remaining ua.in.smartjava.audio frame data to disk.
                    flush();
