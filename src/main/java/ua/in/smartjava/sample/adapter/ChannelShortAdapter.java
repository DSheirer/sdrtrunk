...
public class ChannelShortAdapter implements ISampleAdapter
{
    ...
    public float[] convert( byte[] samples )
    {
		...
		/* Wrap byte array in a byte ua.in.smartjava.buffer so we can process them as shorts */
        ...
        while( buffer.hasRemaining() )
        {
        	if( mMixerChannel == MixerChannel.LEFT )
        	{
            	...
            	/* Throw away the right ua.in.smartjava.channel */
            	buffer.getShort();
        	}
        	else
        	{
            	/* Throw away the left ua.in.smartjava.channel */
            	buffer.getShort();
