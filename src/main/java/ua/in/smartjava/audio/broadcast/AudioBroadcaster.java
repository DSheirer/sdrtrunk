...
public abstract class AudioBroadcaster implements Listener<AudioRecording>
{
    ...
    /**
     * AudioBroadcaster for streaming ua.in.smartjava.audio recordings to a remote streaming ua.in.smartjava.audio server.  Audio recordings are
     * generated by an internal StreamManager that converts an inbound stream of AudioPackets into a recording of the
     * desired ua.in.smartjava.audio format (e.g. MP3) and nominates the recording to an internal recording queue for streaming.  The
     * broadcaster supports receiving ua.in.smartjava.audio packets from multiple ua.in.smartjava.audio sources.  Each ua.in.smartjava.audio packet's internal ua.in.smartjava.audio
     * metadata ua.in.smartjava.source string is used to reassemble each packet stream.  Recordings are capped at 30 seconds length.
     * If a ua.in.smartjava.source ua.in.smartjava.audio packet stream exceeds 30 seconds in length, it will be chunked into 30 second recordings.
     *
     * This broadcaster supports a time delay setting for delaying broadcast of ua.in.smartjava.audio recordings.  The delay setting is
     * defined in the broadcast configuration.  When this delay is greater than zero, the recording will remain in the
     * ua.in.smartjava.audio broadcaster queue until the recording start time + delay elapses.  Audio recordings are processed in a FIFO
     * manner.
     *
     ...
     * there are no recordings available, in order to maintain a connection with the remote server.  Any ua.in.smartjava.audio packet
     * streams received while the broadcaster is stopped will be ignored.
     *
     * The last ua.in.smartjava.audio packet's metadata is automatically attached to the closed ua.in.smartjava.audio recording when it is enqueued for
     * broadcast.  That metadata will be updated on the remote server once the ua.in.smartjava.audio recording is opened for streaming.
     */
    ...
    /**
     * Broadcast binary ua.in.smartjava.audio data frames or sequences.
     */
    ...
    /**
     * Broadcasts the next song's ua.in.smartjava.audio metadata prior to streaming the next song.
     *
    ...
    /**
     * Connects to the remote server specified by the broadcast configuration and starts ua.in.smartjava.audio streaming.
     */
    public void start()
    {
    ...
    /**
     * Number of ua.in.smartjava.audio recordings streamed to remote server
     */
    ...
    /**
     * Number of ua.in.smartjava.audio recordings that were removed for exceeding age limit
     */
    ...
    /**
     * Primary insert method for the stream manager to nominate completed ua.in.smartjava.audio recordings for broadcast.
     *
    ...
    protected void setBroadcastState(BroadcastState state)
    {
        if(mBroadcastState != state)
        {
            ...
            if(!connected())
            {
                //Remove all pending ua.in.smartjava.audio recordings
    ...
    public class RecordingQueueProcessor implements Runnable
    {
            ...
            if(mProcessing.compareAndSet(false, true))
            {
                try
                {
                    ...
                    if(mInputStream != null)
                    {
                        ...
                        {
                            mLog.error("Error reading from in-memory ua.in.smartjava.audio recording input stream", ioe);
                        }
                    }
                    else
                    {
                ...
                {
                    mLog.error("Error while processing ua.in.smartjava.audio streaming queue", e);
                }
        ...
        private void nextRecording()
        {
            ...
            if(nextRecording != null && nextRecording.getStartTime() + mDelay <= System.currentTimeMillis())
            {
                ...
                {
                    mLog.error("Stream [" + getBroadcastConfiguration().getName() + "] error reading temporary ua.in.smartjava.audio " +
