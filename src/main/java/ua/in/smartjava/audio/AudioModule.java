...
import ua.in.smartjava.dsp.filter.polyphase.PolyphaseFIRDecimatingFilter_RB;
...
/**
 * Provides packaging of demodulated ua.in.smartjava.audio ua.in.smartjava.sample buffers into ua.in.smartjava.audio packets for
 * broadcast to registered ua.in.smartjava.audio packet listeners.  Includes ua.in.smartjava.audio packet
 * metadata in constructed ua.in.smartjava.audio packets.
 *
 * Incorporates ua.in.smartjava.audio squelch state listener to control if ua.in.smartjava.audio packets are
    ...
    ISquelchStateListener, Listener<RealBuffer>
{
    ...
    static
    {
        //Band-pass ua.in.smartjava.filter to both decimate (48kHz to 8kHz) and block 0 - 300 Hz (LTR signalling).
        ...
        try
        {
            if(!designer.isValid())
            {
                throw new FilterDesignException("Couldn't design the ua.in.smartjava.audio decimation ua.in.smartjava.filter");
            }
    ...
    public void stop()
    {
        /* Issue an end ua.in.smartjava.audio packet in case a recorder is still rolling */
        if(mAudioPacketListener != null)
        {
    ...
    /**
     * Processes demodulated ua.in.smartjava.audio samples into ua.in.smartjava.audio packets with current ua.in.smartjava.audio
