...
public class BinaryMessage extends BitSet
{
    ...
    /**
     * Pointer to the next fill index location, when adding ua.in.smartjava.bits to this bitset
     * one at a time.
     */
    private int mPointer = 0;

    /**
     * Bitset that buffers ua.in.smartjava.bits added one at a time, up to the size of the this
     * bitset. 
     * 
     ...
     * ua.in.smartjava.message to an EDAC function.
     */
    private CRC mCRC;
    ...
    /**
     * Constructs a bitset ua.in.smartjava.buffer and preloads it with the ua.in.smartjava.bits contained in
     * the bitsToPreload parameter.  If the bitsToPreload are longer than the
     * size of the bitset, only those ua.in.smartjava.bits that fit will be preloaded
     * 
     * @param size
     * @param bitsToPreload
     */
    ...
    /**
     * Returns a mew binary ua.in.smartjava.message containing the ua.in.smartjava.bits from (inclusive) to
     * end (exclusive).
     * 
     * @param start
     * @param end
    ...
    /**
     * Indicates the number of bit errors that were corrected in this ua.in.smartjava.message.
     *
     * @see CorrectedBinaryMessage subclass that supports setting this value
     * @return number of corrected ua.in.smartjava.bits in this ua.in.smartjava.message
     */
    ...
    /**
     * Static method to construct a new BitSetBuffer, preloaded with the ua.in.smartjava.bits
     * from the preload parameter, and then filled with the ua.in.smartjava.bits from the
     * second bitsetbuffer parameter.
     * 
     * @param preloadBits - boolean array of ua.in.smartjava.bits to be prepended to the new
     *          bitset
     * @param bitsetToAppend - full bitset to be appended to the residual ua.in.smartjava.bits array
     * @return - new Bitset preloaded with residual ua.in.smartjava.bits and new bitset
     */
    ...
    /**
     * Clears (sets to false or 0) the ua.in.smartjava.bits in this bitset and resets the
     * pointer to zero.
     */
    @Override
    public void clear()
    ...
    public void add( boolean value ) throws BitSetFullException
    {
        ...
        else
        {
            throw new BitSetFullException( "bitset is full -- contains " + 
            		( mPointer + 1 ) + "ua/in/smartjava/bits");
        }
    }
    ...
    /**
     * Returns this ua.in.smartjava.message as a little endian byte array.  Extra 0 ua.in.smartjava.bits will be padded to the end to make the overall
     * length a multiple of 8.
     * @return little endian ua.in.smartjava.message byte array
     */
    ...
    /**
     * Returns a boolean array of the right-most bitCount number of ua.in.smartjava.bits
     */
    ...
    /**
     * Returns a boolean array representing the ua.in.smartjava.bits located from startIndex
     * through endIndex
     */
     ...
     * 			were contiguous ua.in.smartjava.bits, with index 0 being the MSB and index
    ...
    public int getInt( int[] bits )
    {
    	if( bits.length > 32 )
    	{
    		throw new IllegalArgumentException( "Overflow - must be 32 ua.in.smartjava.bits "
    ...
    public long getLong( int[] bits )
    {
    	if( bits.length > 64 )
    	{
    		throw new IllegalArgumentException( "Overflow - must be 64 ua.in.smartjava.bits "
    ...
    /**
     * Converts up to 63 ua.in.smartjava.bits from the bit array into an integer and then
    ...
    public String getHex( int[] bits, int digitDisplayCount )
    {
    	...
    	else
    	{
    		throw new IllegalArgumentException( "BitSetBuffer.getHex() "
    				+ "maximum array length is 63 ua.in.smartjava.bits" );
    	}
    }
    ...
    public String getHex( int msb, int lsb, int digitDisplayCount )
    {
    	...
    	{
    		throw new IllegalArgumentException( "BitSetBuffer.getHex() "
    				+ "maximum array length is 64 ua.in.smartjava.bits" );
    	}
    }

    /**
     * Returns the int value represented by the bit range.  This method will
     * parse the ua.in.smartjava.bits in big endian or little endian format.  The start value
    ...
    /**
     * Creates a ua.in.smartjava.buffer of size=width and fills the ua.in.smartjava.buffer with the fill value
     * @param width - size of the ua.in.smartjava.buffer
     * @param fill - initial fill value
     * @return - filled ua.in.smartjava.buffer
     */
	...
	/**
	 * Loads the value into the ua.in.smartjava.buffer starting at the offset index, and
	 * assuming that the value represents (width) number of ua.in.smartjava.bits.  The MSB of
	 ...
	 * @param value - value to be loaded into the ua.in.smartjava.buffer
	 */
	...
	/**
	 * Generates an array of ua.in.smartjava.message bit position indexes to support accessing
	 ...
	 * @return - loaded ua.in.smartjava.buffer
	 */
	...
	/**
	 * Left rotates the ua.in.smartjava.bits between start and end indices, number of places.
	 */
	...
	/**
	 * Left rotates the ua.in.smartjava.bits between start and end and wraps the left-most
	...
	/**
	 * Right rotates the ua.in.smartjava.bits between start and end indices, number of places.
	 */
	...
	/**
	 * Right rotates the ua.in.smartjava.bits between start and end and wraps the right-most
	 ...
	 * offset position using width ua.in.smartjava.bits from the value.
	 */
