...
public class R820TTunerController extends RTL2832TunerController
{
    ...
    /**
     * Overrides the same method from the RTL2832 tuner ua.in.smartjava.controller to apply
    ...
    public void setSamplingMode(SampleMode mode) throws LibUsbException
    {
        switch(mode)
        {
            case QUADRATURE:
				...
				/* Enable ua.in.smartjava.spectrum inversion */
    ...
    private void setTVStandard(boolean controlI2C) throws UsbException
    {
        ...
        for(int x = 0; x < 2; x++)
        {
            /* Set ua.in.smartjava.filter cap */
    	...
    	/* Channel ua.in.smartjava.filter extension */
    	...
    	/* RF poly ua.in.smartjava.filter current */
    ...
    /**
     * Assumes value is a byte value and reverses the ua.in.smartjava.bits in the byte.
     */
    ...
    public enum FrequencyDivider
    {
        ...
        public int getSDM(Integral integral, long frequency)
        {
            if(contains(frequency))
            {
                ...
                //Left shift the double value 16 ua.in.smartjava.bits and truncate to an integer
