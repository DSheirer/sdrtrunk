...
import ua.in.smartjava.source.tuner.frequency.FrequencyController;
import ua.in.smartjava.source.tuner.frequency.FrequencyController.Tunable;
...
public abstract class TunerController implements Tunable, IFrequencyChangeProcessor
{
    ...
    /**
     * Abstract tuner ua.in.smartjava.controller class.  The tuner ua.in.smartjava.controller manages frequency bandwidth and currently tuned channels
     ...
     * @param usableBandwidth percentage of usable bandwidth relative to space at the extreme ends of the ua.in.smartjava.spectrum
    ...
    /**
     * Indicates if ua.in.smartjava.channel along with all of the other currently sourced
    ...
    private boolean canTune(TunerChannel channel)
    {
        ...
        if(getMinFrequency() < channel.getMinFrequency() && getMaxFrequency() > channel.getMaxFrequency())
        {
            ...
            else
            {
                ...
                //Requested ua.in.smartjava.channel is within current locked ua.in.smartjava.channel frequency range
                if(minLockedFrequency <= channel.getMinFrequency() && channel.getMaxFrequency() <= maxLockedFrequency)
                {
                    return true;
                }

                //Requested ua.in.smartjava.channel is higher than min locked frequency
                ...
                //Requested ua.in.smartjava.channel is lower than the max locked frequency
    ...
    /**
     * Constructs a digital drop ua.in.smartjava.channel (DDC) as a tuner ua.in.smartjava.channel from from the tuner, or returns null if the ua.in.smartjava.channel
     * cannot be sourced from the tuner.
     *
     * This ua.in.smartjava.controller can't provide a ua.in.smartjava.channel if the ua.in.smartjava.channel, along with the current set of sourced channels, can't be
     ...
     *
     * @param tuner to ua.in.smartjava.source the ua.in.smartjava.channel from
     * @param channel with defined center frequency and bandwidth
     * @return fully constructed tuner ua.in.smartjava.channel or null
    ...
    /**
     * Indicates if the tuner's LO frequency must be updated in order to accommodate the tuner ua.in.smartjava.channel
     */
    ...
    /**
     * Releases the currently sourced tuner ua.in.smartjava.channel from this tuner and shuts down the tuner if no other sources exist.
     */
     ...
     *
     * If there is only a single tuned ua.in.smartjava.channel, it is placed immediately to the right of the usable bandwidth right
     * of the central DC spike.
     *
     * Otherwise, it places the highest ua.in.smartjava.channel frequency at the upper end of the tuner bandwidth, and then iteratively
     ...
     * exception so that the most recently added ua.in.smartjava.channel can be removed.
     *
     * Note: the tuned frequency is not changed until a legitimate new frequency can be calculated.  If an exception
     * is thrown, the current frequency is retained, so that the recently added ua.in.smartjava.channel can be removed and all other
     * channels can continue as previously arranged.
     *
     * @throws SourceException if the set of tuner channels, including a recently added ua.in.smartjava.channel, cannot be tuned
    ...
    private void updateLOFrequency() throws SourceException
    {
        ...
        //If there is only 1 ua.in.smartjava.channel set the center frequency so that the ua.in.smartjava.channel is positioned to the right of center
        ...
        else
        {
            ...
            //Start by placing the highest frequency ua.in.smartjava.channel at the high end of the ua.in.smartjava.spectrum
            candidateFrequency = maxLockedFrequency - (getUsableBandwidth() / 2);

            //Iterate the channels and make sure that none of them overlap the center DC spike ua.in.smartjava.buffer, if one exists
            if(mMiddleUnusable > 0)
            {
                ...
                while(isValidCandidateFrequency && processingRequired)
                {
                    ...
                    //If any of the center ua.in.smartjava.channel(s) overlap the central DC spike avoid area, we'll iteratively
                    //increase the tuned frequency causing the set of channels to move left in the tuned bandwidth until
                    //we either find a good center tune frequency, or we walk the lowest frequency ua.in.smartjava.channel out of the
                    ...
                    for(TunerChannel channel : mTunedChannels)
                    {
                        ...
                        {
                            //Calculate a tuned frequency adjustment that places this overlapping ua.in.smartjava.channel just to the
                            //left of the central DC spike avoid zone
                            long adjustment = channel.getMaxFrequency() - minAvoid + 1;

                            //If the candidate frequency doesn't push the lowest ua.in.smartjava.channel out of bounds, make adjustment
                            if(candidateFrequency + adjustment - (getUsableBandwidth() / 2) <= minLockedFrequency)
                            {
                                candidateFrequency += adjustment;
                                processingRequired = true;
                            }
                            //Otherwise, punt and indicate that we can't find a center frequency or add the ua.in.smartjava.channel
                            else
                            {
    ...
    /**
     * Usable bandwidth - total bandwidth minus the unusable space at either end of the ua.in.smartjava.spectrum.
     */
    ...
    /**
     * Sets the listener to be notified any time that the tuner changes frequency or bandwidth/ua.in.smartjava.sample rate.
     *
