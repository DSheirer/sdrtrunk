...
public class TunerModel extends AbstractTableModel implements Listener<TunerEvent>
{
    ...
    /**
     * Iterates current tuners to get a tuner ua.in.smartjava.channel ua.in.smartjava.source for the frequency
     * specified in the ua.in.smartjava.channel config's ua.in.smartjava.source config object.
     * 
     * Returns null if no tuner can ua.in.smartjava.source the ua.in.smartjava.channel
     */
    public Source getSource( SourceConfigTuner config, int bandwidth )
    {
		...
		while( it.hasNext() && retVal == null )
		{
			...
			{
				mLog.error( "couldn't provide tuner ua.in.smartjava.channel ua.in.smartjava.source", ree );
			}
            catch ( SourceException e )
            {
            	mLog.error( "error obtaining ua.in.smartjava.channel from tuner [" +
            			tuner.getName() + "]", e );
            }
		}
