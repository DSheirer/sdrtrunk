...
import ua.in.smartjava.source.tuner.fcd.proV1.FCD1TunerController.Block;
...
public abstract class FCDTunerController extends TunerController
{
    ...
    /**
     * Generic FCD tuner ua.in.smartjava.controller - contains functionality common across both
                              ...
                              int maxTunableFrequency)
    {
        ...
        {
            mLog.error("Error setting ua.in.smartjava.sample rate to [" + sampleRate + "]", se);
        }
    }

    /**
     * Initializes the ua.in.smartjava.controller by opening the USB device and claiming the
     * HID interface.
     *
     * Invoke this method after constructing this class to setup the
     * ua.in.smartjava.controller.
     *
     ...
     *
     * @param buffer - direct allocated ua.in.smartjava.buffer.  Must be 64 bytes in length.
    ...
    private void write(ByteBuffer buffer) throws LibUsbException
    {
        if(mDeviceHandle != null)
        {
            ...
            if(result != LibUsb.SUCCESS)
            {
                throw new LibUsbException("error writing byte ua.in.smartjava.buffer", result);
            }
        }
        else
        {
     ...
     *
     * @return ua.in.smartjava.buffer read from FCD
     * @throws LibUsbException on error
     */
    private ByteBuffer read() throws LibUsbException
    {
        if(mDeviceHandle != null)
        {
            ...
            if(result != LibUsb.SUCCESS)
            {
                throw new LibUsbException("error reading byte ua.in.smartjava.buffer", result);
            }
    ...
    public class FCDConfiguration
    {
        ...
        /**
         * Extracts the configuration string from the ua.in.smartjava.buffer
         */
