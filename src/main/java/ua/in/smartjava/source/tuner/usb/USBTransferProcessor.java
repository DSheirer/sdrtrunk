 ...
 *
 ******************************************************************************/
package ua.in.smartjava.source.tuner.usb;
...
public class USBTransferProcessor implements TransferCallback
{
    ...
    //Tuner format-specific byte to IQ float ua.in.smartjava.sample converter
                                ...
                                int bufferSize)
    {
        ...
        mFilledBuffers.setOverflowListener(new IOverflowListener()
        {
            ...
            public void sourceOverflow(boolean overflow)
            {
                if(overflow)
                {
                    mLog.debug(mDeviceName + " - ua.in.smartjava.buffer overflow - temporary pause until processing catches up");
                }
                else
                {
                    mLog.debug(mDeviceName + " - ua.in.smartjava.buffer overflow cleared - resuming normal processing");
                }
            }
        });
    }
    ...
    /**
     * Start USB transfer ua.in.smartjava.buffer processing.  Subsequent calls to this method after started will be ignored.
     */
    private void start()
    {
        if(mRunning.compareAndSet(false, true))
        {
            ...
            while(!mAvailableTransfers.isEmpty())
            {
                ...
                if(transfer != null)
                {
                    ...
                    {
                        //TODO: broadcast to each listener that this ua.in.smartjava.source has an error and is shutting down
                        mLog.error(mDeviceName + "- error submitting transfer [" + LibUsb.errorName(result) + "]");
                    }
                }
            }

            //Start transferred ua.in.smartjava.buffer dispatcher
    ...
    /**
     * Stop USB transfer ua.in.smartjava.buffer processing.  Subsequent calls to this method after stopped will be ignored.
     */
    private void stop()
    {
    ...
    /**
     * Indicates if there are complex ua.in.smartjava.buffer listeners registered with this processor
     */
    ...
    private void prepareTransfers() throws LibUsbException
    {
        while(mAvailableTransfers.size() < TRANSFER_BUFFER_POOL_SIZE)
        {
            ...
            if(transfer == null)
            {
                throw new LibUsbException("Couldn't allocate USB transfer ua.in.smartjava.buffer", LibUsb.ERROR_NO_MEM);
            }
    ...
    /**
     * Process a filled transfer ua.in.smartjava.buffer received back from the USB device
     */
    @Override
    public void processTransfer(Transfer transfer)
    {
        ...
        if(mRunning.get())
        {
            ...
            else
            {
                ...
                {
                    mLog.warn(mDeviceName + " - all transfer ua.in.smartjava.buffer processing is stopped");
                    //TODO: no transfers are in progress ... need to alert the user and the registered complex
                    //TODO: ua.in.smartjava.buffer listeners so that they can respond accordingly
                }
            }
        }
        else
    ...
    /**
     * Fetches byte[] chunks from the raw ua.in.smartjava.sample ua.in.smartjava.buffer.  Converts each byte
    ...
    public class BufferDispatcher implements Runnable
    {
        ...
        public void run()
        {
            ...
            while(!mBuffersToDispatch.isEmpty())
            {
                ...
                {
                    mLog.error(mDeviceName + " - error while dispatching complex IQ ua.in.smartjava.buffer samples", e);
                }
