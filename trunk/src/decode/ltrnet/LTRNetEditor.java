...
public class LTRNetEditor extends DecodeEditor
{
    private static final long serialVersionUID = 1L;

    private JComboBox<MessageDirection> mComboDirection;
    
	public LTRNetEditor( DecodeConfiguration config )
	{
		super( config );
	...
	private void initGUI()
	{
		...
		mComboDirection.setSelectedItem( ((DecodeConfigLTRNet)mConfig).getMessageDirection() );
		...
		mComboDirection.addActionListener( new ActionListener() 
		{
            ...
            public void actionPerformed( ActionEvent e )
            {
				...
				if( selected != null )
				{
					((DecodeConfigLTRNet)mConfig).setMessageDirection( selected );
				}
            }
		} );
		
		add( mComboDirection, "span 2,wrap" );
	}
}
