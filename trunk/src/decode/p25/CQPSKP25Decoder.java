...
@Deprecated
public class CQPSKP25Decoder extends Decoder 
			implements DirectFrequencyController, Instrumentable
{
	private final static Logger mLog = LoggerFactory.getLogger( CQPSKP25Decoder.class );
	...
	private ComplexFIRFilter mBasebandFilter = new ComplexFIRFilter( 
		FilterFactory.getLowPass( 48000, 6500, 7500, 48, WindowType.HANNING, true ), 1.0 );
	private CQPSKDemodulator mDemodulator = new CQPSKDemodulator();
	...
	private ComplexAutomaticGainControl mAGC = new ComplexAutomaticGainControl();

	private AliasList mAliasList;
	
	public CQPSKP25Decoder( SampleType sampleType, AliasList aliasList )
	{
		super( sampleType );
		...
		if( mSourceSampleType == SampleType.COMPLEX )
		{
			 ...
			 * to process
			 */
			this.addComplexListener( mAGC );
			mAGC.setListener( mBasebandFilter );
			mBasebandFilter.setListener( mDemodulator );
			mDemodulator.setListener( new Listener<ComplexSample>() 
			{
				@Override
				public void receive( ComplexSample sample )
				{
					mLog.debug( "Output real:" + sample.real() + " imag:" + sample.imaginary() + " angle:" + Math.toDegrees( sample.angle() ) );
				} 
			} );
		}
		else
		{
			throw new IllegalArgumentException( "CQPSK can only process complex samples" );
		}
